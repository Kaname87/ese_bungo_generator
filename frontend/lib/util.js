...
export async function getAllChildrenPagePaths(getAllParentFn, getListFn, fallback=false) {
    ...
    const allPathes = await parentIdList.reduce(async (memo, parentId) => {
        await memo;
        const results = await getChildrenPagePaths(parentId, getListFn);
        await delay(50);
        console.log(cnt+=1)
        return [...(await memo), results];
    }, []);

    const maped = allPathes.map(paths => paths.paths)
    const flatAllPaths = maped.flat();
...
export async function getChildrenPagePaths(parentId, getListFn, fallback=false) {
    // await delay(5000)
    const resultList = await recFetchList(getListFn, parentId)

    const pages = rangeArray(Math.ceil(resultList.length / COUNT_PER_PAGE))
...
export async function getPagePaths(getIdListFn, fallback=false) {
...
export async function recFetchList(fn, id, offset=0, totalList=[]) {
    ...
    const {
        children_list: resultList,
        children_next_offset: nextOffset
    } = res
